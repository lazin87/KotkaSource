...
bool CHttpBrowserSync::checkHttpReqParams()
{
    ...
    if(fResult && (eHttpReqJson == m_eHttpReq) )
    {
        fResult = !m_abDataToSend.isEmpty();
        qDebug() << "Byte Array to send: " << (fResult ? "OK" : "NOK");
    }
...
void CHttpBrowserSync::startHttpRequest(EHttpRequestType a_eHttpReqType)
{
    ...
    else if(eHttpReqJson == a_eHttpReqType)
    {
        oRequest.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
        m_pReplay = m_pNetworkAccessMngr->post(oRequest, m_abDataToSend);
    }
    else
    {
...
void CHttpBrowserSync::setDataToSend(const QByteArray &a_crData)
{
    m_abDataToSend = a_crData;
}
